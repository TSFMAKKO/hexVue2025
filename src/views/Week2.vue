    ...
    <div class="todo-list" v-if="token && userData !== null">
      ...
      <transition-group name="collapse" tag="div">
        <div v-for="todo in todos" :key="todo.id" class="todo-item">
          ...
          <div class="content">
            <!-- 顯示文字 -->
            <span v-if="!todo.isEdit" :class="{ del: todo.status }" @dblclick="todo.isEdit = true">
              {{ todo.content }}
            </span>

            <!-- 編輯輸入框 -->
            <input v-else type="text" :value="todo.content" @keypress.prevent.enter="updateText(todo.id, $event)"
              @keydown.esc="todo.isEdit = false" />
          </div>

          <button v-if="todo.isEdit" type="button" @click="todo.isEdit = false">
            取消
          </button>
          <button v-else type="button" @click="deleteHandler(todo.id, $event)">
            刪除
          </button>
        </div>
      </transition-group>
